...
st.set_page_config(page_title="Camera 3", layout='wide')
...
st.markdown(f"<div class='kpi'><h2 style='margin:0'>Camera 3 Dashboard</h2><div class='small'>Live feed & controls</div></div>", unsafe_allow_html=True)

video_source = st.text_input("Video source (leave to use default)", "traffic3.mp4")
...
source = video_source if video_source else "traffic3.mp4"
cap = None
...
if cap and cap.isOpened():
    ...
    with st.spinner('Processing video…'):
        while True:
            ...
            log_stats("Camera_3", total, 1 if counts.get('accident') else 0, counts.get('ambulance',0))
            frames += 1
            if frames % 8 == 0:
                agg = aggregate_stats()
                if agg is not None:
